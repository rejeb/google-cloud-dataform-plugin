...
public class BqGridLoader implements GridLoader {
    ...
    @Override public void updateTotalRowCount(@NotNull GridRequestSource source)    {}
    @Override public void applyFilterAndSorting(@NotNull GridRequestSource source)  {}
    @Override public void updateIsTotalRowCountUpdateable()                         {}
    ...
    private void applyRows(@NotNull List<FieldValueList> rows, @NotNull GridRequestSource source) {
        ...
        for (int i = 0; i < rows.size(); i++) {
            gridRows.add(new BqGridRow((int) (offset + i + 1), rows.get(i)));
        }
        model.replaceRows(gridRows);
        source.getActionCallback().setDone();
        if (hookUp != null) {
            hookUp.firePagingChanged();
        }
    }
}
